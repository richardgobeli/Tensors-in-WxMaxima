...
;Displays P([L1],[L2],I1,I2,...) using three separate slots: a subscript
...
;; A blank standing in for a missing index must be as wide as the real
;; index at that position on the other row, or later indices drift and
...
(defun ishow (f)
  (cond
    ((atom f) f)
    ((rpobj f) (ishow-classic f))
    (t (cons (car f) (mapcar (quote ishow) (cdr f))))))
...
(defun ishow-classic (f)
  (let* ((name (caar f))
         ...
         (contra-raw (cdaddr f))
         (deriv (cdddr f))
         (sub-tokens nil) (super-tokens nil))
    (dolist (idx cov-raw)
      (if (ishow-negated-index idx)
          (progn (push (ishow-blank (caddr idx)) sub-tokens)
                 (push (caddr idx) super-tokens))
          (progn (push idx sub-tokens)
                 (push (ishow-blank idx) super-tokens))))
    (dolist (idx contra-raw)
      (if (ishow-negated-index idx)
          (progn (push (caddr idx) sub-tokens)
                 (push (ishow-blank (caddr idx)) super-tokens))
          (progn (push (ishow-blank idx) sub-tokens)
                 (push idx super-tokens))))
